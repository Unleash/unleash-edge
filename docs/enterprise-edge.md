...
Enterprise Edge is the commercial build of Unleash Edge. It extends the open-source runtime with additional functionality for licensed Unleash Enterprise installations. It runs the same commands and configuration as the OSS binary, but requires a valid Enterprise license to start and access its extended capabilities.
...
All CLI arguments and environment variables documented for Unleash Edge apply to this image. Replace `<version>` with the version you want to run. We generally recommend staying up to date with the latest release. Available tags can be viewed on
...
- **Edge observability:** Enterprise Edge reports heartbeat and instance state to Unleash Enterprise. This data is displayed in the management UI to assist with monitoring replica health, token usage, and deployment status.
