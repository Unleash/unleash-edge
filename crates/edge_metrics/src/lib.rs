...
use unleash_edge_types::metrics::{DESIRED_URLS, ENDPOINT_LABEL, METHOD_LABEL, STATUS_LABEL};
use utoipa::ToSchema;
    ...
    pub fn observe(
        ...
        for family in registry.gather().iter() {
            match family.get_name() {
                unleash_edge_types::metrics::HTTP_REQUESTS_DURATION => {
                    family
                        .get_metric()
                        .iter()
                        .filter(|m| {
                            m.has_histogram()
                                && m.get_label().iter().any(|l| {
                                    l.get_name() == ENDPOINT_LABEL
                                        && DESIRED_URLS
                                            .iter()
                                            .any(|desired| l.get_value().ends_with(desired))
                                })
                                && m.get_label().iter().any(|l| {
                                    l.get_name() == STATUS_LABEL && l.get_value() == "200"
                                        || l.get_value() == "202"
                                        || l.get_value() == "304"
                                        || l.get_value() == "403"
                                })
                        ...
                        .for_each(|m| {
                            ...
                            let path = labels
                                .iter()
                                .find(|l| l.get_name() == ENDPOINT_LABEL)
                                .unwrap()
                                .get_value()
                            ...
                            let method = labels
                                .iter()
                                .find(|l| l.get_name() == METHOD_LABEL)
                                .unwrap()
                                .get_value();
                            let status = labels
                                .iter()
                                .find(|l| l.get_name() == STATUS_LABEL)
                                .unwrap()
                                .get_value();
