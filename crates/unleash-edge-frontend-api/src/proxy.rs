...
#[instrument(skip(app_state, edge_token, connect_info, context))]
pub async fn post_proxy_all_features(
    app_state: State<AppState>,
    edge_token: EdgeToken,
    connect_info: ConnectInfo<SocketAddr>,
...
pub async fn proxy_post_metrics(
    app_state: State<AppState>,
    edge_token: EdgeToken,
...
pub async fn proxy_register_client(
    app_state: State<AppState>,
    edge_token: EdgeToken,
...
pub async fn proxy_get_enabled_features(
    app_state: State<AppState>,
    edge_token: EdgeToken,
...
#[instrument(skip(app_state, edge_token, client_ip, context))]
pub async fn proxy_post_enabled_features(
    app_state: State<AppState>,
    edge_token: EdgeToken,
    client_ip: ConnectInfo<SocketAddr>,
    context: Json<Context>,
...
pub fn router(disable_all_endpoint: bool) -> Router<AppState> {
    let mut base_router = Router::new()
        .route(
            "/proxy",
            get(proxy_get_enabled_features).post(proxy_post_enabled_features),
        )
    ...
    if !disable_all_endpoint {
        base_router = base_router
            .route(
                "/proxy/all",
                get(get_proxy_all_features).post(post_proxy_all_features),
            )
