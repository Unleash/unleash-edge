    ...
    impl EdgePersistence for S3Persister {
        ...
        async fn save_tokens(&self, tokens: Vec<EdgeToken>) -> EdgeResult<()> {
            ...
            self.client
                ...
                .await
                .map(|_| ())
                .map_err(|err| EdgeError::PersistenceError(format!("Failed to save tokens: {err:?}")))
        }
        ...
        async fn save_features(&self, features: Vec<(String, ClientFeatures)>) -> EdgeResult<()> {
            ...
            {
                Ok(_) => Ok(()),
                Err(s3_err) => Err(EdgeError::PersistenceError(format!("Failed to save features: {s3_err:?}")))
            }
        }
        ...
        async fn save_license_state(&self, license_state: &LicenseState) -> EdgeResult<()> {
            ...
            {
                Ok(_) => Ok(()),
                Err(s3_err) => Err(EdgeError::PersistenceError(format!("Failed to save license state: {s3_err:?}")))
            }
        }
        ...
        async fn save_last_event_ids(&self, event_ids: HashMap<String, u32>) -> EdgeResult<()> {
            ...
            {
                Ok(_) => Ok(()),
                Err(s3_err) => Err(EdgeError::PersistenceError(format!("Failed to save last event ids: {s3_err:?}")))
            }
        }
