...
pub async fn register(app_state: State<AppState>, edge_token: EdgeToken, client_application: Json<ClientApplication>) -> impl IntoResponse {
    ...
    Response::builder().status(StatusCode::ACCEPTED).header("X-Edge-Version", EDGE_VERSION).body(Body::empty()).unwrap()
}
