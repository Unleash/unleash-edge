...
use unleash_edge_types::metrics::DESIRED_URLS;
...
impl Default for LabelsConfiguration {
    fn default() -> Self {
        Self {
            endpoint: unleash_edge_types::metrics::ENDPOINT_LABEL.to_string(),
            method: unleash_edge_types::metrics::METHOD_LABEL.to_string(),
            status: unleash_edge_types::metrics::STATUS_LABEL.to_string(),
            version: None,
        }
    }
}
...
impl Default for ActixMetricsConfiguration {
    fn default() -> Self {
        Self {
            http_requests_total_name: unleash_edge_types::metrics::HTTP_REQUESTS_TOTAL.to_string(),
            http_server_duration_milliseconds: unleash_edge_types::metrics::HTTP_REQUESTS_DURATION
                .to_string(),
            http_response_size_name: unleash_edge_types::metrics::HTTP_RESPONSE_SIZE.to_string(),
        ...
        Poll::Ready(Ok(res.map_body(move |head, body| {
            ...
            if inner.matches(&path, &method) {
                ...
                if let Some(instance_data) = edge_instance_data {
                    if let Some(endpoint) = DESIRED_URLS.into_iter().find(|u| path.ends_with(u)) {
