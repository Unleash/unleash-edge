...
use crate::{EdgeToken, Status, UnleashBadRequest};
...
#[cfg(feature = "s3-persistence")]
impl From<AwsSdkError<GetObjectError>> for EdgeError {
    fn from(err: AwsSdkError<GetObjectError>) -> Self {
        EdgeError::PersistenceError(format!("failed to get object {}", err))
    }
}

#[cfg(feature = "s3-persistence")]
impl From<AwsSdkError<PutObjectError>> for EdgeError {
    fn from(err: AwsSdkError<PutObjectError>) -> Self {
