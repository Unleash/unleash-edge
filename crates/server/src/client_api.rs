...
use unleash_edge_types::cli::{EdgeArgs, EdgeMode};
use unleash_edge_types::error::EdgeError;
use unleash_edge_types::tokens::cache_key;
use unleash_edge_types::{
    BatchMetricsRequestBody, EDGE_VERSION, EdgeJsonResult, EdgeResult, EdgeToken, FeatureFilters,
};
...
#[get("/delta")]
#[instrument(skip(edge_token, token_cache, filter_query, req))]
...
pub async fn register(
    ...
    unleash_edge_client_metrics::register_client_application(
        edge_token,
        &connect_via,
    ...
    Ok(HttpResponse::Accepted()
        .append_header(("X-Edge-Version", EDGE_VERSION))
        .finish())
}
...
pub async fn metrics(
    ...
    unleash_edge_client_metrics::register_client_metrics(
        edge_token,
        metrics.into_inner(),
        metrics_cache,
    );
...
pub async fn post_bulk_metrics(
    ...
    unleash_edge_client_metrics::register_bulk_metrics(
...
mod tests {
    ...
    use maplit::hashmap;
    use ulid::Ulid;
    use unleash_edge_client_metrics::{ApplicationKey, MetricsBatch, MetricsKey};
    use unleash_edge_types::cli::{OfflineArgs, TokenHeader};
    use unleash_edge_types::{EDGE_VERSION, TokenType, TokenValidationStatus};
    ...
    async fn bulk_metrics_endpoint_correctly_refuses_metrics_without_auth_header() {
        ...
        let status = req
            ...
            .body(
                serde_json::to_string(&BatchMetricsRequestBody {
                    applications: vec![],
                    metrics: vec![],
                })
                .unwrap(),
    ...
    async fn register_endpoint_returns_version_header() {
        ...
        assert_eq!(res.headers().get("X-Edge-Version").unwrap(), EDGE_VERSION);
    }
