...
use unleash_edge::http::background_send_metrics;
...
use unleash_edge::http::instance_data::{InstanceDataSending, loop_send_instance_data};
use unleash_edge::http::refresher::feature_refresher::FeatureRefresher;
use unleash_edge::http::unleash_client::ClientMetaInformation;
use unleash_edge::offline::offline_hotload;
use unleash_edge::persistence::{EdgePersistence, persist_data};
use unleash_edge::{
    client_api, edge_api, frontend_api, health_checker, internal_backstage, openapi, prom_metrics,
    ready_checker, tls,
};
use unleash_edge_client_metrics::MetricsCache;
use unleash_edge_metrics::EdgeInstanceData;
use unleash_edge_prometheus_remote_writer::prometheus_remote_write;
use unleash_edge_types::cli::{CliArgs, EdgeMode};
use unleash_edge_types::{EdgeToken, TokenValidationStatus};
    ...
    match schedule_args.mode {
        EdgeMode::Edge(edge) => {
            ...
            tokio::select! {
                ...
                _ = background_send_metrics::send_metrics_task(metrics_cache_clone.clone(), refresher.clone(), edge.metrics_interval_seconds.try_into().unwrap()) => {
                ...
                _ = prometheus_remote_write(prom_registry_for_write.clone(), edge.prometheus_remote_write_url, edge.prometheus_push_interval, edge.prometheus_username, edge.prometheus_password, app_name) => {
                    info!("Prometheus push unexpectedly shut down");
                }
                _ = loop_send_instance_data(instance_data_sender.clone(), our_instance_data.clone(), downstream_instance_data.clone()) => {
