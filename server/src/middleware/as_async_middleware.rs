...
/// Middleware transform for [`as_async_middleware`].
...
mod tests {
    ...
    impl MyMw {
            ...
            B: MessageBody + 'static,
        {
            ...
            as_async_middleware(move |req, next| {
    ...
    async fn compat_compat() {
        let _ = App::new().wrap(Compat::new(as_async_middleware(noop)));
        let _ = App::new().wrap(Compat::new(as_async_middleware(mutate_body_type)));
    }
    ...
    async fn feels_good() {
        let app = test::init_service(
            App::new()
                .wrap(as_async_middleware(mutate_body_type))
                .wrap(as_async_middleware(add_res_header))
                .wrap(Logger::default())
                .wrap(as_async_middleware(noop))
