...
impl InstanceDataSender {
    pub fn from_args(args: CliArgs, instance_data: Arc<EdgeInstanceData>) -> Result<Option<Self>, EdgeError> {
        match args.mode {
            EdgeMode::Edge(edge_args) => {
                ...
                Ok(edge_args.tokens.first().map(|token| {
                    ...
                    let unleash_client = Url::parse(&edge_args.upstream_url.clone())
                        ...
                        .map(Arc::new)
                        .map_err(|_| EdgeError::InvalidServerUrl(edge_args.upstream_url.clone())).expect("Could not construct UnleashClient");
                    Self {
                        unleash_client,
                        token: token.clone(),
                    }
...
pub async fn send_instance_data(
    ...
    loop {
        ...
        if let Some(instance_data_sender) = instance_data_sender.clone() {
            ...
            if do_the_work {
                let status = instance_data_sender.unleash_client
                    .send_instance_data(
                        observed_data,
                        &instance_data_sender.token,
                    )
                    .await;
                match status {
                    Ok(_) => {}
