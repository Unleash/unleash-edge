mod delta_test {
    ...
    use chrono::Duration;
    use dashmap::DashMap;
    use unleash_types::client_features::{ClientFeature, ClientFeatures, ClientFeaturesDelta, Constraint, Operator, Segment};
    ...
    async fn test_delta() {
        ...
        let refreshed_features = features_cache.get(&cache_key(&token)).unwrap().value().clone();
        assert_eq!(refreshed_features, initial_features);

        let token_refresh = feature_refresher.tokens_to_refresh.get(&token.token).unwrap();
        feature_refresher.refresh_single_delta(token_refresh.value().clone()).await;
        let refreshed_features = features_cache.get(&cache_key(&token)).unwrap().value().clone();
    ...
    async fn return_client_features_delta(etag_header: Option<String>) -> HttpResponse {
        match etag_header {
            Some(value) => match value.as_str() {
                "1" => HttpResponse::Ok().insert_header(ETag(EntityTag::new_strong("2".to_string()))).json(revision(2)),
                "2" => HttpResponse::NotModified().finish(),
                _ => HttpResponse::NotModified().finish(),
            },
            None => HttpResponse::Ok().insert_header(ETag(EntityTag::new_strong("1".to_string()))).json(revision(1))
        }
    ...
    async fn test_features_server() -> TestServer {
        test_server(move || {
            HttpService::new(map_config(
                App::new()
                    .service(
                        web::resource("/api/client/delta").route(web::get().to(|req: HttpRequest| {
                            let etag_header = req
                                .headers()
                                .get(IF_NONE_MATCH)
